...
@bot.on.raw_event(GroupEventType.MESSAGE_EVENT, MessageEvent,
                  PayloadMapRule({"columns": int, "upper": int, "lower": int}))
async def set_format(event: Union[MessageEvent, Message]):
    if event.peer_id not in glue_users:
        return
    ...
    if isinstance(event, MessageEvent):
        await event.edit_message(message="Выбери рамки", keyboard=boards)
    else:
        ...
        message = await event.answer(message="Выбери рамки", keyboard=boards)
...
async def set_boards(event: Union[Message, MessageEvent]):
    ...
    if isinstance(event, MessageEvent):
        await event.edit_message(message="🫖 Чайник достаёт свой волшебный клей")
    else:
        message: MessagesSendUserIdsResponseItem = glue_users[event.peer_id]['message']
        await bot.api.messages.delete([message.message_id], delete_for_all=True)
        message = await event.answer(message="🫖 Чайник достаёт свой волшебный клей")
        glue_users[event.peer_id]['message'] = message
    ...
    if not await bot.api.groups.is_member(group_id=GROUP_ID, user_id=event.peer_id):
    ...
    await bot.api.messages.send(peer_id=event.peer_id, message=reply, attachment=photo)
