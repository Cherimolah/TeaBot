...
class MarkovMiddleware(BaseMiddleware[Message], ABC):
    ...
    async def post(self):
        if (self.event.peer_id > 2_000_000_000
                ...
                and self.event.from_id > 0
                # and not self.event.transliterated
                and await db.select([db.Chat.generation_mode]).where(db.Chat.chat_id == self.event.chat_id).gino.scalar()
        ):
